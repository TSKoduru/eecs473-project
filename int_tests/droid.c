...
//  TODO: in future iteration of the board the XSHUT signal should have a 10k pull up resistor
  ...
  //  printf("VL53L1X_CalibrateXtalk=%d\n", xtalk/512);
    ...
    TIM_SetPWM(MIN_DUTY, MIN_DUTY, MIN_DUTY, MIN_DUTY);
      ...
      xyz_ground.x = 0; xyz_ground.y = 0; xyz_ground.z = 0;
      for(int i = 0; i < 5; i++) {
  ...
  while (1)
  {
	...
	//			  printf("No data ready for long time, please check your system\n");
				  char buffer[6] = {'e', 'r', 'r', 'o', 'r', '\n'};
				  HAL_UART_Transmit(&hlpuart1, &buffer, 6, 0xFFFF);
				  timeout_counter = 0;
			  }
